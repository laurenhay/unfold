...
class Unfolder:
    def __init__(self, groomed, closure = False, herwig_closure = False, do_syst = False, cms_label = "Internal", filename = None):
        ...
        non_jes_sys_list = ['nominal', 'LuminosityDown', 'PDFDown', 'L1prefiringDown', 'Q2muRDown', 'Q2muFDown', 'PUSFDown', 'JMRDown', 'JMSDown', 'herwigDown', 'LuminosityUp', 'PDFUp', 'L1prefiringUp', 'Q2muRUp', 'PUSFUp', 'Q2muFUp', 'JMRUp', 'JMSUp', 'herwigUp', ]
        print(os.getcwd())
        filename_sys = 'inputs/sys_matrix_dic.pkl'
        ...
        if do_syst:
            ...
            self.systematics = ['nominal']#, 'herwigUp', 'herwigDown', 'JMRUp']
        ...
        print("edges pt reco", self.pt_edges, ' len ', len(self.pt_edges))
        print("edges reco", self.edges, ' len ', len(self.edges))
        print("edges gen", self.edges_gen, ' len ', len(self.edges_gen))
        self._load_data(filename_mc = '../coffeaOutput/dijetHists_wXS_HTplots_QCDsim_MG__allUncALLRespOnly.pkl', filename_data = "../coffeaOutput/dijetHists_fixGenWeights_JetHT_allUncALLRespOnly.pkl", filename_herwig='../coffeaOutput/dijetHists_wXS_HTplots_QCDsim_herwig__allUncALLRespOnly.pkl') #pythia in herwig as dummy, replace with actual herwig file
    ...
    def _load_data(self, filename_mc = '../coffeaOutput/dijetHists_wXS_HTplots_QCDsim_MG__allUncALLRespOnly.pkl', filename_data = "../coffeaOutput/dijetHists_fixGenWeights_JetHT_allUncALLRespOnly.pkl", filename_herwig='../coffeaOutput/dijetHists_wXS_HTplots_QCDsim_herwig__allUncALLRespOnly.pkl'):
        print(filename_mc)
        ...
        with open( filename_sys, "rb") as f:
            sys_matrix_dic = pkl.load( f )
        print(output_pythia.keys())
        if self.groomed:
            pythia4d = output_pythia['response_matrix_g']
            herwig4d = output_herwig['response_matrix_g']
            pythia2d = output_pythia['ptreco_mreco_g']
            herwig2d = output_herwig['ptreco_mreco_g']
            data2d = output_data['ptreco_mreco_g']
            pythia_gen2d = output_pythia['ptgen_mgen_g']
            herwig_gen2d = output_herwig['ptgen_mgen_g']
        else:
            pythia4d = output_pythia['response_matrix_u']
            herwig4d = output_herwig['response_matrix_u']
            pythia2d = output_pythia['ptreco_mreco_u']   
            herwig2d = output_herwig['ptreco_mreco_u']
            data2d = output_data['ptreco_mreco_u']
            pythia_gen2d = output_pythia['ptgen_mgen_u']
            herwig_gen2d = output_herwig['ptgen_mgen_u']
        ...
        fakes = pythia2d.project('ptreco', 'mreco', 'syst') + (-1)*pythia4d.project('ptreco', 'mreco', 'syst')
        fakes_herwig = herwig2d.project('ptreco', 'mreco', 'syst') + (-1)*herwig4d.project('ptreco', 'mreco', 'syst')
        self.fakes = fakes
        self.fakes_herwig = fakes_herwig
        misses = pythia_gen2d.project('ptgen', 'mgen', 'syst') + (-1)*pythia4d.project('ptgen', 'mgen', 'syst')
        misses_herwig = herwig_gen2d.project('ptgen', 'mgen', 'syst') + (-1)*herwig4d.project('ptgen', 'mgen', 'syst')
        self.misses = misses
        ...
        print("Pythia resp matrix mreco edges", resp_matrix_4d.project('mreco').axes[0].edges)
        print("Pythia resp matrix mgen edges", resp_matrix_4d.project('mgen').axes[0].edges)
        print("Pythia resp matrix otgen edges", resp_matrix_4d.project('ptgen').axes[0].edges)
        pt_edges        = self.bins.pt_edges
        mass_edges_reco = self.bins.mass_edges_reco
        mass_edges_gen  = self.bins.mass_edges_gen
        print("edges reco rebinned", mass_edges_reco, 'len', len(mass_edges_reco))
        print("edges gen rebinned", mass_edges_gen, 'len', len(mass_edges_gen))
        print("Loaded pkl files and rebinned histograms.")
        for syst in self.systematics:
            print("doing syst ", syst)
            if (syst == "herwigUp") or (syst == "herwigDown"):
                print("Processing Herwig syst:", syst)
                resp_matrix_4d_syst = resp_matrix_4d_herwig[{'syst':'nominal'}]
                proj = resp_matrix_4d_syst.project('ptreco', 'mreco', 'ptgen', 'mgen')
                M_np = proj.values(flow=False)
                print("M np shape ", M_np.shape)
                ...
                print("herwig 2d fakes ", self.h2d_fakes_herwig.shape)
            ...
            if syst == 'nominal':
                ...
                resp_matrix_4d_syst_gen = resp_matrix_4d_gen[{'syst':syst}]
    ...
    def plot_unfolded(self, log = False):
        ...
        for i in range(len(self.pt_edges)-1):
            ...
            hep.histplot(unfolded_pt_binned[i]/bin_widths/unfolded_pt_binned[i].sum(), self.bins.gen_mass_edges_by_pt[i], yerr = error_pt_binned[i]/bin_widths/unfolded_pt_binned[i].sum(), label = 'Unfolded Herwig' if self.herwig_closure else 'Unfolded', color = 'k', ls = '--' )
    ...
    def plot_systematic_fraction(self, syst_name = 'all'):
        ...
        for i, result in enumerate(self.normalized_results):
            ...
            if jes_up:
                ...
                hep.histplot(jes_up_frac, self.bins.gen_mass_edges_by_pt[i], label="JES ")
            ...
            if jer_up:
                ...
                hep.histplot(jer_up_frac, self.bins.gen_mass_edges_by_pt[i], label="JER ")
            ...
            if ele_up:
                ...
                hep.histplot(ele_up_frac, self.bins.gen_mass_edges_by_pt[i], label="Electron SFs")
            ...
            if mu_up:
                ...
                hep.histplot(mu_up_frac, self.bins.gen_mass_edges_by_pt[i], label="Muon SFs")
            ...
            for syst in self.systematics:
                ...
                if syst.endswith('Up'):
                    ...
                    if syst.startswith('herwig'):
                        ...
                        hep.histplot(syst_fraction_up, self.bins.gen_mass_edges_by_pt[i], label=f"{syst[:-2]}")
    ...
    def plot_systematic_frac_indiv(self, syst_names = ['JES' , 'JER']):
        ...
        for i, result in enumerate(self.normalized_results):
            ...
            for syst in syst_names:
                ...
                if up_key in syst_fraction_dict:
                    hep.histplot(syst_fraction_dict[up_key], self.bins.gen_mass_edges_by_pt[i], label=f"{syst} Up", color=color, ls='-')
                # Plot Down uncertainty (dashed)
                if down_key in syst_fraction_dict:
                    hep.histplot(-syst_fraction_dict[down_key], self.bins.gen_mass_edges_by_pt[i], label=f"{syst} Down", color=color, ls='--')
            if pt_bin[1] == float('inf') or pt_bin[1] > 100000:
                pt_bin_label = f"{pt_bin[0]}–∞"
            else:
                pt_bin_label = f"{pt_bin[0]}–{pt_bin[1]}"
            ...
            if self.groomed:
                ...
                plt.savefig(f'outputs/plots/uncertainties/{syst_names[0]}_groomed_{i}.pdf', dpi=300)
            else:
                plt.xlim(10,250)
                plt.xlabel("Ungroomed Jet Mass (GeV)")
                plt.savefig(f'outputs/plots/uncertainties/{syst_names[0]}_ungroomed_{i}.pdf', dpi=300)
    ...
    def plot_correlation(self):
        ...
        hep.cms.label(self.cms_label, data=True)
        plt.tight_layout()
        plt.savefig(f'outputs/plots/unfold/correlation_groomed.pdf' if self.groomed else f'outputs/plots/unfold/correlation_ungroomed.pdf')
